...
def deepsort_usingTorchtoloadYOLO():
    # - SET UP: DeepSORT
    opt = parse_opt()
    cfg = get_config()
    cfg.merge_from_file(opt.config_deepsort)
    ...
    video = cv2.VideoCapture('E:/TEST/0929/DeepSort/videos/360p.mp4')

    model = torch.hub.load('ultralytics/yolov5', 'custom', 'E:/TEST/0929/DeepSort/best.pt') #base: yolov5n
    model.eval()
    names = model.names
    ...
    while True:
        ...
        for output in outputs:
                    ...
                    cvzone.putTextRect(frame, f' {int(track_id)}{str(names[class_id])}', (max(0, x1),max(35,y1)), 
                                scale=1, thickness=1, offset=0)                
...
if __name__ == '__main__':
    t1 = time_sync()
    deepsort_usingTorchtoloadYOLO()
    t2 = time_sync()
    # with torch.no_grad():
    #     yolov5_deepsort_tracking()
    # t3 = time_sync()

    print("Time run: {:.3f}s".format(t2-t1))